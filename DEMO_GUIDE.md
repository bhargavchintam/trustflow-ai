...
5. **Live eval dashboard.** 35 cases, four categories, all at 100% on the deployed URL. p50/p95 latency and cost per request shown next to the pass rates.
...
A. 35 cases across four categories — routing (13), security (9), memory (7), tenant_isolation (6). Edge cases include case-insensitivity, XML tag injection, social engineering, role spoofing, in-prompt tenant override, and fake header injection. All currently at 100% on the deployed URL.
...
- [ ] `/eval` shows current 35/35 result. If stale, run `python -m app.evals.run_evals --api $LIVE_URL` from your laptop first.
